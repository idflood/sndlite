...
;(function($) {
  ...
  $.scPlayer = function(options, node) {
    var opts = $.extend({}, $.scPlayer.defaults, options),
        ...
        if( links.length ) loadTracksData($player, links, opts.apiKey);
  ...
  $.scPlayer.setTrack = function(player, target) {
    ...
    var link = links[0];
    /*console.log(scApiUrl(link.url, opts.apiKey));
    
    $.getJSON(scApiUrl(link.url, opts.apiKey), function(data) {
      data.permalink_url = link.url;
      console.log("loaded");
      console.log(data);
      updateTrackInfo( player, data );
